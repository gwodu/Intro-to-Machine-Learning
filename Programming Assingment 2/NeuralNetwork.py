...
def fit_NeuralNetwork(X_train,y_train,alpha,hidden_layer_sizes,epochs):
    ...
    weight_layer= np.random.normal(0, 0.1, (hidden_layer_sizes[(L-4) +1]+1,1)) # changed l to L- 4 to avoid index out of range
    weights.append(weight_layer) 
    ...
    for e in range(epochs):
        ...
        for n in range(N):
            ...
            currX, currS = forwardPropagation(x, weights)
            delta = backPropagation(currX, y_train[index], currS, weights)
            weights = updateWeights(weights, delta, alpha)
            errN += (currX[-1] - y_train[index])**2
        err[e]=errN/N 
    return err, weights
...
def forwardPropagation(x, weights):
    ...
    for i in range(l-1):
        ...
        currS= np.dot(currX, weights[i]) #: Dot product between the layer and the weight matrix
        retS.append(currS)
        currX=currS
        if i != len(weights)-1:
            for j in range(len(currS)):
                currX[j]=  activation(currS[j]) # : Apply the activation
...
def errorPerSample(X,y_n):
    #: Follow the instruction in the document
    ...
    for i in range(len(X)-2, 0, -1): #L-1,...,0
        ...
        for j in range(len(s[i-1])): #number of nodes in layer i - 1
            for k in range(len(s[i])): #number of nodes in layer i
                ...
                delN[j]=delN[j]+delNextLayer[k]*WeightsNextLayer[k][0]*derivativeActivation(sCurrLayer[j])# Fill in the rest
...
def derivativeError(x_L,y):
    ...
    if y==1:
        ...
        return -(y / (x_L + epsilon)) + ((1 - y) / (1 - x_L + epsilon))
...
def test_SciKit(X_train, X_test, Y_train, Y_test):
    ...
    mlp = MLPClassifier(hidden_layer_sizes=(30,10), solver= 'adam', alpha=10**5,  random_state=1)
